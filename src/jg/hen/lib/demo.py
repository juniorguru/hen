...
async def _fetch_demo(http: httpx2.AsyncClient, homepage_url: str) -> httpx2.Response:
...
async def fetch_demo(
    http: httpx2.AsyncClient, homepage_url: str
) -> httpx2.Response | Exception:
    try:
...
def get_demo_url(result: httpx2.Response | Exception | None) -> str | None:
    if not result:
        return None
